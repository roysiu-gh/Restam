...
class Restaurant(object):
    ...
    def get_data( self, iden:int ) -> dict:
        try:
            if iden in self.in_progress_transactions:
                data_to_get = self.in_progress_transactions[iden]
                completed = False
            elif iden in self.completed_transactions:
                data_to_get = self.completed_transactions[iden]
                completed = True
            else: raise AssertionError( "id found stored in self.in_progress_transactions or self.completed_transactions" )
        except AssertionError as e:
            print( "Error in {}".format(sys._getframe().f_code.co_name), e.args )
            raise # Re-raise error for handling
        else:
            return { "data":data_to_get, "completed":completed }
    
    def finish_party( self, iden:int ) -> None:
        try:
            assert iden in self.in_progress_transactions , "id not found in self.in_progress_transactions"
            assert iden not in self.completed_transactions , "id found in self.completed_transactions, contact system admin"
        except AssertionError as e:
            print( "Error in {}".format(sys._getframe().f_code.co_name), e.args )
            raise # Re-raise error for handling
        else:
    ...
    def search_bookings( self, category, search_term ):
        pass
